...
const StuAllPaper = () => {
  ...
  useEffect(() => {
    dispatch(fetchAllPapersAsync());
     ...
     const colors =['bg-orange-500','bg-gray-700','bg-sky-500','bg-purple-500','bg-green-500','bg-red-500','bg-yellow-500','bg-pink-500','bg-blue-600']
  ...
  return (
    ...
    <section className="grid grid-cols-[repeat(auto-fit,minmax(260px,1fr))] gap-8">
      {allPapers.map((data, idx) => (
        ...
        >
          {/* Top Badge */}
          <div className={`w-12 h-12 mx-auto flex items-center justify-center rounded-full  text-white text-lg font-bold mb-3 shadow-sm ${colors[idx % colors.length]}`}>
            {idx + 1}
          </div>
          ...
          {/* Button */}
         <Link to={`test/${data._id}`}> <button className="mt-3 px-5 py-2 border border-purple-600 text-purple-600 font-medium rounded-lg hover:bg-purple-600 hover:text-white transition-all duration-300 cursor-pointer">
            Start Exam
          </button></Link>
        </div>
      ))}
